...
def run():
    ...
    check("DISTILL_LR is float", isinstance(training.DISTILL_LR, float), f"{training.DISTILL_LR}")
    check(
        "DISTILL_LR <= LORA_LR (gentler)",
        training.DISTILL_LR <= training.LORA_LR,
        f"{training.DISTILL_LR} <= {training.LORA_LR}",
    )
    check(
        "DISTILL_EPOCHS is int",
        isinstance(training.DISTILL_EPOCHS, int),
        f"{training.DISTILL_EPOCHS}",
    )
    check(
        "DISTILL_REPLAY_RATIO in [0,1]",
        0 <= training.DISTILL_REPLAY_RATIO <= 1,
        f"{training.DISTILL_REPLAY_RATIO}",
    )
    check(
        "DISTILL_WARMUP_RATIO >= LORA_WARMUP_RATIO",
    ...
    check(
        "Empty queries returns ok with 0 generated",
    ...
    check("Unknown teacher returns error", result.get("status") == "error")
    ...
    check(
        "Anthropic teacher without key returns error",
    ...
    try:
        ...
        check(
            "Teacher generation handles Ollama gracefully",
    ...
    try:
        ...
        check(
            "Blend total is reasonable", result.get("total", 0) > 0, f"total={result.get('total')}"
        )
        check(
            "Blend breakdown has both sources",
        ...
        check("Duplicate output name returns error", result2.get("status") == "error")
        ...
        check("Nonexistent source returns error", result3.get("status") == "error")
        ...
        check(
            "Same seed produces same breakdown",
        ...
        for ds in [
            ...
            ds_b,
            ds_out,
            "deltai-verify-blend-out2",
            "deltai-verify-blend-out3",
            "deltai-verify-blend-out4",
            "deltai-verify-blend-out5",
        ]:
            path = training._dataset_path(ds)
            if os.path.exists(path):
                os.remove(path)
    ...
    check(
        "Retention baseline queries defined",
    ...
    check(
        "verify_retention has correct params",
        "model_name" in sig.parameters
        and "baseline_model" in sig.parameters
        and "min_pass_rate" in sig.parameters,
    )
    ...
    try:
        ...
        check(
            "Distill rejects empty teacher dataset",
    ...
    check("Distill rejects nonexistent teacher", result.get("status") == "error")
