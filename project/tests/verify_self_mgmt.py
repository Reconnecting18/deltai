...
def test_he_cb_events():
    ...
    try:
        ...
        cb_events = [e for e in list(main._health_events)[initial_count:]
        ...
        # Reset via success
        main._cb_success()
        cb_events2 = [e for e in list(main._health_events)[initial_count:]
...
def test_ml_sim_start():
    ...
    assert main._resource_state["pending_14b_preload"] == False
test_ml_sim_start()
...
def test_ml_sim_stop():
    ...
    if not sim_active and prev_sim:
        ...
        main._resource_state["pending_14b_preload"] = True
    assert main._resource_state["pending_14b_preload"] == True
...
def test_int_resource_state():
    import main
    required = ["last_vram_action", "last_recovery", "vram_warnings",
                "ollama_failures", "watcher_restarts", "actions_taken",
                "last_sim_state", "sim_stop_detected_at", "pending_14b_preload"]
...
def test_int_tools_intact():
    ...
    assert len(TOOLS) == len(EXECUTORS), f"Mismatch: {len(TOOLS)} tools vs {len(EXECUTORS)} executors"
