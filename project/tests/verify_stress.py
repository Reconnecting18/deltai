...
def verify_fix_3():
    ...
    assert result.get("skipped") == "rate-limited", \
        "Rate limiting should skip rapid calls"
verify_fix_3()
...
def verify_fix_4():
    ...
    assert "text_path" in src, \
        "Should write to temp text file before passing to PowerShell"
verify_fix_4()
...
def verify_fix_5():
    ...
    assert "journal_mode=WAL" not in connect_src, \
        "_connect() should NOT set WAL pragma"
    assert "journal_mode=WAL" in init_src, \
        "init_db() should set WAL pragma"
verify_fix_5()
...
def verify_fix_7():
    ...
    assert "except OSError" in src, \
        "list_datasets should catch OSError on os.path.getsize"
verify_fix_7()
...
def verify_fix_8():
    ...
    assert hasattr(anthropic_client, 'ANTHROPIC_VERSION'), \
        "ANTHROPIC_VERSION constant should exist"
    assert anthropic_client.ANTHROPIC_VERSION == "2023-06-01", \
        "API version should be 2023-06-01"
verify_fix_8()
...
def stress_tier_classification():
    ...
    assert router.classify_complexity("") == 1 or router.classify_complexity("") in (1, 2)  # empty is fine as tier 1
    # Complex engineering should be tier 3
    assert router.classify_complexity("derive the stress-strain equation for this beam deflection") == 3
    ...
    long_msg = "I need help understanding " + "something really complex " * 15 + "? how does this work and why is it so difficult?"
...
def stress_concurrent_routing():
    ...
    threads = []
    messages = [
        "hello", "analyze telemetry", "what is 2+2",
        "derive stress equation", "read my file",
        "pit strategy for next stint", "system status",
    ]
    for msg in messages:
...
def stress_auto_capture():
    ...
    r = training.auto_capture("test-auto", "explain the router",
