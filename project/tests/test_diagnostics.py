...
def test_diag_ollama():
    ...
    assert "model" in result.lower() or "DOWN" in result, \
        "Should mention models or DOWN status"
test_diag_ollama()
...
def test_diag_gpu():
    ...
    assert "VRAM" in result or "unavailable" in result.lower(), \
        "Should mention VRAM or unavailable"
test_diag_gpu()
...
def test_repair_watcher():
    ...
    assert "OK" in result or "ERROR" in result, \
        f"Should return OK or ERROR: {result}"
    if "OK" in result:
