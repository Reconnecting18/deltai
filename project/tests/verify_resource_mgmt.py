...
def test_cb_backoff():
    ...
    main._cb_success()  # reset
    main._circuit_breaker["backoff_sec"]
    # Trigger open
    for _ in range(main._CB_FAILURE_THRESHOLD):
        main._cb_failure()
    ...
    assert second_backoff <= main._CB_MAX_BACKOFF, (
        f"Backoff {second_backoff} exceeds max {main._CB_MAX_BACKOFF}"
    )
...
def test_stress_cb_inference():
    ...
    async def run():
        async with __import__("httpx").AsyncClient(timeout=5) as client:
            data, err = await main._try_ollama_inference(
                client, "deltai-test", [{"role": "user", "content": "test"}]
            )
            return data, err
...
def test_stress_vram_critical():
    ...
    assert main._resource_state["vram_warnings"] >= 2, "Should accumulate VRAM warnings"
