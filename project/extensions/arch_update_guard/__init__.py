"""
Arch update guard (optional extension): Arch news/wiki, pacman evidence, snapshots, diffs, rollback.

Routes are registered from setup(); TOOLS lists LLM tool schemas (lightweight import).
On main, this tree is typically absent (gitignore); on personal, force-track with git add -f.
"""
...
def setup(app) -> None:
    ...
    mount_all(app)
    logger.info("arch_update_guard: extension initialised")
